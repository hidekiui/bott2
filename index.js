...
// ANTI LINK GRUP
		        ...
		        if (isGroupAdmins) return reply('Seu pau é grande?:v')
		        baby.updatePresence(from, Presence.composing)
		        if (mesejAnti.includes("#izinbos")) return reply("Sem spam no grupo porra")
...
*/
				...
				if (isBanned) return reply(nad.baned())
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Envie fotos / videos / gifs com legendas \n${prefix}sticker (duração do vídeo de 1 a 9 segundos)`)
					}
					break
				case 'nulis':
                  if (isBanned) return reply(nad.baned())
		...
		case 'textlight':
		    ...
		    break
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, 'Onde está o código do idioma Sis? exemplo : ${prefix}tts id Olá Screamo', text, {quoted: mek})
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (!isQuotedSticker) return reply('Responda ou marque o sticker que você deseja fazer uma imagem rs')
					reply(nad.wait())
...
*/		 
				case 'welcome':
                  if (isBanned) return reply(nad.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('*「 SUCESSO 」ATIVANDO O RECURSO DE BEM-VINDO NO GRUPO*')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/group/welkom.json', JSON.stringify(welkom))
						reply('*「  SUCESSO 」DESATIVANDO O RECURSO DE BEM-VINDO NO GRUPO*')
					} else {
						reply(nad.satukos())
					}
					break
                 ...
                 case 'event':
                  if (isBanned) return reply(nad.baned())                 
					...
					if (Number(args[0]) === 1) {
						...
						event.splice(from, 1)
						fs.writeFileSync('./database/group/event.json', JSON.stringify(event))
						reply('*「 SUCESSO 」DESATIVAR MORTAL EM GRUPO*')
					} else {
						reply(nad.satukos())
					}
					break
                ...
                if (args.length < 1) return reply('Ekhemm >_<')
                if (args[0] === '1') {
                    if (isLevelingOn) return reply('*O RECURSO DE DE LEVEL JÁ ESTAVA ATIVO*')
                    _leveling.push(from)
				...
				case 'nsfw':
                  if (isBanned) return reply(nad.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(nsfw))
						reply('*「  」SUCESSO」DESLIGADONDO O RECURSO NSWF DO GRUPO*')
					} else {
						reply(nad.satukos())
					}
					break
                  ...
                  if (isBanned) return reply(nad.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						baby.sendMessage(from,`𝘼𝙇𝙀𝙍𝙏𝘼!!! Se nao ser negro supremo nao envie links de grupo`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntiLink) return reply('SÓ ESPERAR?')
						var ini = anti.botLangsexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('「 SUCESSO 」ANTI LINK DESLIGADO DO GRUPO')
					} else {
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))			
					if (!isGroup) return reply(nad.groupo())
					teks = `*LISTA DOS NEGROS SUPREMOS:* _${groupMetadata.subject}_\n*TOTAL* : ${groupAdmins.length}\n\n`
					no = 0
				...
				if (!isRegistered) return reply(nad.noregis())	
					...
					try {
						...
						console.log('Error :', e)
						reply('Eita, deu erro, talvez o número seja privado :)')
					}
					break
			     	case 'kick':
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())	     	
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Selecione o negro comum que você quer banir')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Pedido aceito removido deste grupo , motivo muito claro estava usando calcinha bege e isto é broxante🏃 :\n`
							teks += `@_.split('@')[0]`
						}
...
*/
                ...
                addKoinUser('35932540387@s.whatsapp.net', fee)
